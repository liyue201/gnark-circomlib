...
func Montgomery2Edwards(api frontend.API, in []frontend.Variable) []frontend.Variable {
	...
	out[1] = api.Div(api.Sub(in[0], 1), api.Add(in[0], 1))

	//api.AssertIsEqual(api.Mul(out[0], in[1]), in[0])
...
func MontgomeryDouble(api frontend.API, in []frontend.Variable) []frontend.Variable {
	...
	x1_2 := api.Mul(in[0], in[0])
	lamda := api.Div(api.Add(api.Mul(3, x1_2), api.Mul(2, aa, in[0]), 1), api.Mul(2, bb, in[1]))
	...
	out[1] = api.Sub(api.Mul(lamda, api.Sub(in[0], out[0])), in[1])
